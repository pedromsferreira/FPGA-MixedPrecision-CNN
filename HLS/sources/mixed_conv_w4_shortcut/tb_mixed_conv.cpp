...
#define TB_OUT_SCALE_MAP1 4
#define TB_OUT_SCALE_MAP2 4
#define TB_MAP2_SIGNED 1

//char*	weights_file = "/home/pferreira/deepL/pytorch-cifar/weightsExtractor/output/0003layer1[1].conv1.weight.bin";
char*	weights_file = "/home/pferreira/deepL/pytorch-cifar/weightsExtractor/output/0007layer2[0].shortcut[0].weight.bin";
...
char*	acts_file = "/home/pferreira/deepL/pytorch-cifar/maps/layer1_block0_relu2_sh.bin";
char*	acts_file_python = "/home/pferreira/deepL/pytorch-cifar/maps/layer1_block0_relu1.bin";
char*	acts_file_sum = "/home/pferreira/HwSw/HLS/outputs/output_L2_B0_C2_noReLU_hw.bin";
...
char*   output_map_file = "/home/pferreira/HwSw/HLS/outputs/output_filler.bin";
char*   output_map_file_sw = "/home/pferreira/HwSw/HLS/outputs/output_filler_sw.bin";
...
static  ap_int<DMA_WIDTH> inputMap2[TB_OUTPUT_MAP_SIZE*TB_OUTPUT_MAP_SIZE*((TB_NUM_FILTERS-1)/NUM_ACTS_IN_STREAM+1)];
...
void read_map_simple(int bitWidth, char *mapFile, int nFilters, int nChannels, int mapSize) {
	...
	if(DO_SHORTCUT){
		for(int m = 0, counter = 0; m < TB_OUTPUT_MAP_SIZE*TB_OUTPUT_MAP_SIZE*((TB_NUM_FILTERS-1)/NUM_ACTS_IN_STREAM+1); m++) {
			fread(buffer2, sizeof(char), 16, fp2);
			for(int i = 15; i >= 0; i--) {
				buf2 = buf2 << 8;
				if(counter < TB_NUM_FILTERS) {
					buf2 = buf2 | (buffer2[i]&0xFF);
				}
...
hls::stream<stream_in_t> str_in_map2("inputStream");
...
int main()
{
	...
	//		printf("%X\n",tmp.data.range((DMA_WIDTH/4)-1,0).to_int());
		...
		if(DO_SHORTCUT){
			for (int y =0 ;y<TB_OUTPUT_MAP_SIZE*TB_OUTPUT_MAP_SIZE*((TB_NUM_FILTERS-1)/NUM_ACTS_IN_STREAM+1); y++) {
				tmp.data=(ap_int<DMA_WIDTH>)inputMap2[y];
		...
		//		printf("%X\n",tmp.data.range((DMA_WIDTH/4)-1,0).to_int());
				...
				str_in_map2.write(tmp);
			}
			printf("\nSent whole Input Map2 N = %d  \n\n",TB_OUTPUT_MAP_SIZE*TB_OUTPUT_MAP_SIZE*((TB_NUM_FILTERS-1)/NUM_ACTS_IN_STREAM+1));
		}

		// 0-2: padding, 3-5: kernelSize
		int ctrl=(TB_PADDING)+(TB_KERNEL_SIZE<<3)+(TB_STRIDE<<6)+(TB_BIAS_SCALE<<8)+
				 (TB_MAP_SIGNED_OK<<12)+(TB_RELU_OK<<13)+(TB_SCALE<<14)+
				 (DO_SHORTCUT<<17)+(TB_OUT_SCALE_MAP1<<18)+(TB_OUT_SCALE_MAP2<<21)+
				 (TB_MAP2_SIGNED<<24);
		conv(str_in, str_out, str_in_map2,TB_NUM_FILTERS,TB_NUM_KERNEL,TB_INPUT_MAP_SIZE,TB_INPUT_MAP_SIZE,ctrl);
