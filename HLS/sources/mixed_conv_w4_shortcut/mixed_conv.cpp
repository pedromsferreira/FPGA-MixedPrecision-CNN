...
void read_stream_activations(act_bus_t *inputMap,
							 unsigned int initAddr,
...
//	printf("\nReading ACTIVATIONS from stream...\n\n");
	...
	#pragma HLS loop_tripcount min=(34*3*3) max=(34*3*3)
	#pragma HLS PIPELINE
...
void read_weights(weight_bus_t filter[NUM_PE][MAX_WEIGHTS_MEM_SIZE],
	...
	for(int i = 0; i < totalWeightStreams; i++) {
		#pragma HLS pipeline
		#pragma HLS loop_tripcount min=(64*64/32) max=(64*64/32)
		// if(currPE != 0) {
		// 	debug_OK = false;
		// }
		// else {
...
void conv(hls::stream<stream_in_t> &strm_in,
	...
	//ctrl bus splitting
	// 0-2: padding, 3-5: kernelSize, 6-7: stride, 8-11: biasScale, 12: mapSigned, 13: relu, 14-16: scale, 17: shortcut
