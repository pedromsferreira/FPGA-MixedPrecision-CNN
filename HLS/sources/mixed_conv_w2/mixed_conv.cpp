...
//	printf("\nReading ACTIVATIONS from stream...\n\n");
	...
	//#pragma HLS loop_tripcount min=1 max=(MAX_IMAGE_SIZE*MAX_FEATURE_MAP_LINES*3)
	#pragma HLS PIPELINE
		...
		for(int x = 0; x < outMapSizeX; x++) {
			...
			for(int f = 0; f < filtersPerPE; f++) {
				...
				for(int ky = 0; ky < kernelSize; ky++) {
					#pragma HLS loop_tripcount min=MAX_KERNEL_SIZE max=MAX_KERNEL_SIZE
					KernelXLOOP:
					for(int kx = 0; kx < kernelSize; kx++) {
						#pragma HLS loop_tripcount min=MAX_KERNEL_SIZE max=MAX_KERNEL_SIZE
						ChannelLOOP:
						for(int kn = 0; kn < numKernels; kn+=SIMD_ITERS) {
							#pragma HLS loop_tripcount min=(512/SIMD_ITERS) max=(512/SIMD_ITERS)
							...
							if(kn == 0) {
								//stride = 1 だけな条件
//								if(stride > 1 && ky == kernelSize-1 && kx == kernelSize-stride && read_OK == false) {
//									read_OK = (readLines < inputMapSizeY);
//									streamsRead = streamsPerInputLine-stride*streamsPerAct;
//								}
							...
							if(ky==kernelSize-1 && kx==kernelSize-1) {
								if(kn + SIMD_ITERS >= numKernels) {
									...
									for(unsigned short peId = 0; peId < NUM_PE; peId++) {
										...
										//limit boundary
										if(accum[peId] >= ((1<<(ACTS_WIDTH-1))-1)) {
											accum[peId] = (1<<(ACTS_WIDTH-1))-1;
										}
										else if(accum[peId] <= -((1<<(ACTS_WIDTH-1))-1)) {
											accum[peId] = -((1<<(ACTS_WIDTH-1))-1);
										}
									}
