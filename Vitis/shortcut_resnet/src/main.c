...
XAxiDma DMA_W4, DMA_W4_SH;
XAxiDma DMA_W2, DMA_W2_SH;
...
int conv(int weightWidth,
    ...
    if(weightWidth == 4) {
        ...
        *(int *)(XPAR_XCONV_0_S_AXI_BUS1_BASEADDR + XCONV_BUS1_ADDR_INPUTMAPSIZEY_DATA) = inputMapSizeY;
        ...
        int ctrlBus = (padding) + (kernelSize << 3) + (stride << 6) + 
                      (biasScale << 8) + (isMapSigned << 12) + (relu << 13) + 
                      (scale << 14) + (shortcut << 17) + (outScaleMap1 << 18) +
                      (outScaleMap2 << 21) + (isMapSigned2 << 24);
    ...
    if(weightWidth == 4) {
        ...
        if(shortcut == 1){
        	TxBufferPtr = (uintptr_t *) inMapAddr2;
            status = XAxiDma_SimpleTransfer(&DMA_W4_SH, (UINTPTR) TxBufferPtr, totalStreamsPerZValuesOut*numOutValuesPerFM*DMA_WIDTH_IN_B, XAXIDMA_DMA_TO_DEVICE);
            if(status != XST_SUCCESS) {
              	int stat = XST_FAILURE;
                printf("Error: DMA input map 2 transfer schedule failed\n");
                	return status;
            }
            while (XAxiDma_Busy(&DMA_W4_SH, XAXIDMA_DMA_TO_DEVICE)) { /* Wait for Tx*/ }
            //printf("Sent input feature map 2! \r\n");
        }
        ...
        if(shortcut == 1){
            TxBufferPtr = (uintptr_t *) inMapAddr2;
            status = XAxiDma_SimpleTransfer(&DMA_W2_SH, (UINTPTR) TxBufferPtr, totalStreamsPerZValuesOut*numOutValuesPerFM*DMA_WIDTH_IN_B, XAXIDMA_DMA_TO_DEVICE);
            if(status != XST_SUCCESS) {
                printf("Error: DMA input map 2 transfer schedule failed\n");
                return status;
            }
            while (XAxiDma_Busy(&DMA_W2_SH, XAXIDMA_DMA_TO_DEVICE)) { /* Wait for Tx*/ }
        }
...
int init_DMAs() {
    ...
    XAxiDma_Config *CfgPtr_W2, *CfgPtr_W2_sh;
    XAxiDma_Config *CfgPtr_W4, *CfgPtr_W4_sh;
    

    CfgPtr_W2 = XAxiDma_LookupConfig(XPAR_AXI_DMA_W2_DEVICE_ID);
    CfgPtr_W2_sh = XAxiDma_LookupConfig(XPAR_AXI_DMA_W2_SH_DEVICE_ID);
    CfgPtr_W4 = XAxiDma_LookupConfig(XPAR_AXI_DMA_W4_DEVICE_ID);
    CfgPtr_W4_sh = XAxiDma_LookupConfig(XPAR_AXI_DMA_W4_SH_DEVICE_ID);
    ...
    else if(!CfgPtr_W2_sh) {
        printf("Error: DMA_W2_SH device config not found\r\n");
        return XST_FAILURE;
    }
    else if(!CfgPtr_W4_sh) {
        printf("Error: DMA_W4_SH device config not found\r\n");
        return XST_FAILURE;
    }
    ...
    status = XAxiDma_CfgInitialize(&DMA_W2_SH, CfgPtr_W2_sh);
    if(status != XST_SUCCESS) {
        printf("Error: DMA_W2_SH initialization failed\r\n");
        return XST_FAILURE;
    }
    if(XAxiDma_HasSg(&DMA_W2_SH)) {
        printf("Error: DMA_W2_SH configured as SG mode\r\n");
        return XST_FAILURE;
    }

    status = XAxiDma_CfgInitialize(&DMA_W4_SH, CfgPtr_W4_sh);
    if(status != XST_SUCCESS) {
        printf("Error: DMA_W4_SH initialization failed\r\n");
        return XST_FAILURE;
    }
    if(XAxiDma_HasSg(&DMA_W4_SH)) {
        printf("Error: DMA_W4_SH configured as SG mode\r\n");
        return XST_FAILURE;
    }
    ...
    XAxiDma_IntrDisable(&DMA_W2_SH, XAXIDMA_IRQ_ALL_MASK, XAXIDMA_DEVICE_TO_DMA);
    XAxiDma_IntrDisable(&DMA_W2_SH, XAXIDMA_IRQ_ALL_MASK, XAXIDMA_DMA_TO_DEVICE);

    XAxiDma_IntrDisable(&DMA_W4_SH, XAXIDMA_IRQ_ALL_MASK, XAXIDMA_DEVICE_TO_DMA);
    XAxiDma_IntrDisable(&DMA_W4_SH, XAXIDMA_IRQ_ALL_MASK, XAXIDMA_DMA_TO_DEVICE);
...
uintptr_t BasicBlock(int weightWidth,
    ...
    uintptr_t inMapAddr;
    uintptr_t outMapAddr;
    uintptr_t inMap2Addr;
    int shortcut = (stride != 1 || inChannels != numFilters);
    ...
    inMap2Addr = PADDING_ADDR;
     XTime_GetTime(&tStart);
     status = conv(weightWidth, numFilters, inChannels,
                   kernelSize, inputMapSizeX, inputMapSizeY, stride,
                   1, isMapSigned, 0, ACT_SCALE[scale_idx], 1, 0, 0, 0, 0,
                   weightAddr, biasAddr, inMapAddr, outMapAddr, inMap2Addr);
     XTime_GetTime(&tEnd);
     timeDiff = tEnd - tStart;
//     printf("%.3f ms\n", (float) timeDiff/(COUNTS_PER_SECOND) * 1000);
//     if(status != 0){
...
//    printf("%.3f\n", (float) timeDiff/(COUNTS_PER_SECOND) * 1000);
    ...
    if(shortcut) {
        ...
        inMap2Addr = inMapAddr_start;
        isMapSum = 1;
    }
     XTime_GetTime(&tStart);
     status = conv(weightWidth, numFilters, numFilters,
                   kernelSize, outMapSizeX, outMapSizeY, 1,
                   1, isMapSigned, 0, ACT_SCALE[scale_idx+1], 1,
                   isMapSum, RELU_SCALE[reluScale_idx+1], RELU_SCALE[reluScale_idx-1],
                   0, weightAddr, biasAddr, inMapAddr, outMapAddr, inMap2Addr);
     XTime_GetTime(&tEnd);
     timeDiff = tEnd - tStart;
    // printf("%.3f ms\n", (float) timeDiff/(COUNTS_PER_SECOND) * 1000);
    // if(status != 0){
    ...
    if(shortcut) {
       ...
       a = (int8_t *) (outMapAddr);
        ...
        outMapAddr += outMapMemAlloced;
         XTime_GetTime(&tStart);
         status = conv(weightWidth, numFilters, inChannels,
                       1, inputMapSizeX, inputMapSizeY, stride, 0, isMapSigned,
                       0, ACT_SCALE[scale_idx+2], 1, 1, RELU_SCALE[reluScale_idx+1],
                       RELU_SCALE[reluScale_idx+1], 0, weightAddr, biasAddr,
                       inMapAddr, outMapAddr, inMap2Addr);
         XTime_GetTime(&tEnd);
         timeDiff = tEnd - tStart;
//         printf("%.3f ms\n", (float) timeDiff/(COUNTS_PER_SECOND) * 1000);
//         if(status != 0){
...
int ResNet18() {
    ...
    //Conv
     ...
     status = conv(4, 64, IMAGE_DEPTH, 3,
                   ...
                   1, 0, 0, 0, 0, weightAddr, biasAddr,
                   inMapAddr, outMapAddr, PADDING_ADDR);
     XTime_GetTime(&tEnd);
     timeDiff = tEnd - tStart;
//     printf("%.3f ms\n", (float) timeDiff/(COUNTS_PER_SECOND) * 1000);
     if(status != 0){
